...
export type PortalOrder={
  ...
  paymentMethod:string;paymentAccountTitle:string;paymentAccountNumber:string;
  paymentInstructions:string;paymentTransactionId:string;paymentProofSubmitted:boolean;
  paymentSubmittedAt:string|null;paymentVerifiedAt:string|null;paymentRejectionReason:string;
...
function toPortalOrder(value:Record<string,unknown>):PortalOrder{
  ...
  return {
    ...
    paymentAccountTitle:String(value.paymentAccountTitle??""),
    paymentAccountNumber:String(value.paymentAccountNumber??""),
    paymentInstructions:String(value.paymentInstructions??""),
    paymentTransactionId:String(value.paymentTransactionId??""),
    paymentProofSubmitted:Boolean(value.paymentProofPublicId||value.paymentScreenshotUrl),
    paymentSubmittedAt:value.paymentSubmittedAt?new Date(String(value.paymentSubmittedAt)).toISOString():null,
    paymentVerifiedAt:value.paymentVerifiedAt?new Date(String(value.paymentVerifiedAt)).toISOString():null,
    paymentRejectionReason:String(value.paymentRejectionReason??""),
