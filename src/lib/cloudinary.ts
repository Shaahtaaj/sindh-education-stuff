...
export function createPrivateImageUrl(publicId:string,format?:string){
  if(!isCloudinaryConfigured())throw new Error("Cloudinary credentials are not configured.");
...
export async function uploadPrivatePaymentProof(file:File):Promise<UploadApiResponse>{
  if(!isCloudinaryConfigured())throw new Error("Cloudinary credentials are not configured.");
  const buffer=Buffer.from(await file.arrayBuffer());
  return new Promise((resolve,reject)=>{
    const stream=cloudinary.uploader.upload_stream({
      folder:"sindh-education-stuff/payment-proofs",
      resource_type:"image",
      type:"private",
      use_filename:false,
      unique_filename:true
    },(error,result)=>{
      if(error||!result){reject(error??new Error("Payment proof upload failed."));return}
      resolve(result);
    });
    stream.end(buffer);
  });
...
export async function deletePrivatePaymentProof(publicId:string){
