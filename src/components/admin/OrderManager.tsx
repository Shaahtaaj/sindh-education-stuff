...
async function uploadDeliveryFile(file:File){
  if(file.size>5*1024*1024)throw new Error(`${file.name} exceeds the 5 MB limit.`);
  const authResponse=await fetch("/api/uploads",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"signature",kind:"document",purpose:"delivery"})});
  const auth=await authResponse.json();if(!authResponse.ok)throw new Error(auth.error??"Unable to authorize upload.");
  const data=new FormData();data.set("file",file);data.set("timestamp",String(auth.timestamp));data.set("folder",auth.folder);data.set("signature",auth.signature);data.set("api_key",auth.apiKey);data.set("type",auth.deliveryType);
  const response=await fetch(`https://api.cloudinary.com/v1_1/${encodeURIComponent(auth.cloudName)}/raw/upload`,{method:"POST",body:data});
  const result=await response.json();if(!response.ok)throw new Error(result.error?.message??`Unable to upload ${file.name}.`);
  return {name:file.name,url:String(result.secure_url),publicId:String(result.public_id),format:String(result.format??file.name.split(".").pop()??""),resourceType:String(result.resource_type??"raw"),deliveryType:String(result.type??"private")};
}
...
export function OrderManager() {
  ...
  const [deliveryUploads,setDeliveryUploads]=useState<File[]>([]);
  ...
  async function deliver(){
    if(!selected||!deliveryUploads.length)return;
    setSaving(true);setError("");
    try{
      const uploaded=await Promise.all(deliveryUploads.map(uploadDeliveryFile));
      const deliveryFiles=[...(selected.deliveryFiles??[]),...uploaded];
      const response=await fetch("/api/orders",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:selected._id,deliveryFiles,orderStatus:"completed"})});
      const body=await response.json();if(!response.ok)throw new Error(body.error??"Unable to deliver files.");
      const updated=body as Order;setOrders(current=>current.map(order=>order._id===updated._id?updated:order));setSelected(updated);setDeliveryUploads([]);setNotice("Delivery files published to the customer portal.");
