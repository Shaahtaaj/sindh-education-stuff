...
type MaterialRecord = {
  ...
  metaDescription: string; files: MaterialFile[]; fileUrl: string; fileName: string;
...
const emptyRecord = (): MaterialRecord => ({
  ...
  metaDescription:"",files:[],fileUrl:"",fileName:"",thumbnailUrl:"",
...
function MultiFileField({files,onFiles,existing,onRemoveExisting}:{files:File[];onFiles:(files:File[])=>void;existing:MaterialFile[];onRemoveExisting:(index:number)=>void}) {
  const input=useRef<HTMLInputElement>(null);
  function addFiles(list:FileList|null){
    if(!list)return;
    const incoming=Array.from(list);
    onFiles([...files,...incoming.filter(file=>!files.some(current=>current.name===file.name&&current.size===file.size))]);
  }
  return <div className="md:col-span-2"><p className="mb-2 text-xs font-extrabold uppercase tracking-[.06em] text-[#53657a]">Resource files</p><button type="button" onClick={()=>input.current?.click()} onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();addFiles(event.dataTransfer.files)}} className="flex w-full items-center gap-4 rounded-2xl border-2 border-dashed border-[#cfd9e1] bg-[#f8fafb] p-5 text-left transition hover:border-[#92bda3] hover:bg-[#f3f9f5]"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white text-[#147a4b] shadow-sm"><UploadCloud size={21}/></span><span><span className="block text-sm font-bold text-[#0b1f3a]">Add assignment files or supporting notes</span><span className="mt-1 block text-xs text-[#718094]">PDF, DOC, DOCX, PNG or JPG · max 5 MB each</span></span><input ref={input} className="hidden" type="file" multiple accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" onChange={event=>{addFiles(event.target.files);event.target.value=""}}/></button>{existing.length||files.length?<div className="mt-3 grid gap-2">{existing.map((file,index)=><div key={`${file.url}-${index}`} className="flex items-center gap-3 rounded-xl border border-[#dfe6ec] bg-white px-3 py-2.5"><FileArchive size={17} className="shrink-0 text-[#147a4b]"/><span className="min-w-0 flex-1 truncate text-sm font-semibold text-[#405169]">{file.name}</span><span className="text-[10px] font-bold uppercase text-[#7a899a]">Saved</span><button type="button" onClick={()=>onRemoveExisting(index)} aria-label={`Remove ${file.name}`} className="grid size-7 place-items-center rounded-lg text-[#8a5960] hover:bg-red-50"><X size={15}/></button></div>)}{files.map((file,index)=><div key={`${file.name}-${file.size}-${index}`} className="flex items-center gap-3 rounded-xl border border-[#cde6d6] bg-[#f3f9f5] px-3 py-2.5"><FileArchive size={17} className="shrink-0 text-[#147a4b]"/><span className="min-w-0 flex-1 truncate text-sm font-semibold text-[#405169]">{file.name}</span><span className="text-[10px] font-bold uppercase text-[#147a4b]">New</span><button type="button" onClick={()=>onFiles(files.filter((_,fileIndex)=>fileIndex!==index))} aria-label={`Remove ${file.name}`} className="grid size-7 place-items-center rounded-lg text-[#8a5960] hover:bg-red-50"><X size={15}/></button></div>)}</div>:null}</div>;
}
...
export function MaterialStudio() {
  ...
  function openNew(){const record=emptyRecord();record.program=programs[0]??"";record.semester=semesters[0]??"";const firstCourse=courseRecords.find(course=>(!record.program||course.program===record.program)&&(!record.semester||course.semester===record.semester));record.course=firstCourse?`${firstCourse.code} — ${firstCourse.name}`:"";setDraft(record);setDocumentFiles([]);setThumbnailFile(null);setError("");setEditor(true)}
  function openEdit(record:MaterialRecord){const files=Array.isArray(record.files)&&record.files.length?record.files:record.fileUrl?[{name:record.fileName||"Download",url:record.fileUrl}]:[];setDraft({...record,files});setDocumentFiles([]);setThumbnailFile(null);setError("");setEditor(true)}
  ...
  async function save(event:React.FormEvent){
    ...
    setSaving(true);
    try{
      const [newFiles,newThumbnailUrl]=await Promise.all([Promise.all(documentFiles.map(async file=>({name:file.name,url:await uploadFile(file)}))),uploadFile(thumbnailFile)]);
      const files=[...draft.files,...newFiles];
      const saved={...draft,id:draft.id||crypto.randomUUID(),files,fileUrl:files[0]?.url??"",fileName:files[0]?.name??"",thumbnailUrl:newThumbnailUrl||draft.thumbnailUrl,updatedAt:new Intl.DateTimeFormat("en-PK",{day:"numeric",month:"short",year:"numeric"}).format(new Date())};
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="material-editor-title" className="flex h-full w-full max-w-[850px] flex-col bg-[#f4f7f9] shadow-[-24px_0_70px_rgba(3,15,30,.24)]">
        ...
        <form onSubmit={save} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 space-y-5 overflow-y-auto p-4 md:p-6">
            ...
            <section className="rounded-2xl border border-[#dfe6ec] bg-white p-5 shadow-[0_5px_20px_rgba(11,31,58,.03)] md:p-6"><div className="mb-5 flex items-center gap-3"><span className="grid size-8 place-items-center rounded-lg bg-[#0b1f3a] text-xs font-extrabold text-white">2</span><div><h3 className="text-sm font-extrabold text-[#0b1f3a]">Files and access</h3><p className="mt-0.5 text-xs text-[#7a899a]">Attach multiple student downloads and one visual cover.</p></div></div><div className="grid gap-5 md:grid-cols-2"><MultiFileField files={documentFiles} onFiles={setDocumentFiles} existing={draft.files} onRemoveExisting={index=>update("files",draft.files.filter((_,fileIndex)=>fileIndex!==index))}/><DropField label="Thumbnail image" kind="image" accept=".png,.jpg,.jpeg" file={thumbnailFile} onFile={setThumbnailFile} existingName={draft.thumbnailUrl?"Current thumbnail":undefined}/><div className="md:col-span-2"><p className="mb-2 text-xs font-extrabold uppercase tracking-[.06em] text-[#53657a]">Access type</p><div className="grid grid-cols-2 gap-3">{(["Free","Paid"] as const).map(option=><button type="button" onClick={()=>update("access",option)} key={option} className={`rounded-xl border px-4 py-3 text-left transition ${draft.access===option?"border-[#147a4b] bg-[#eaf6ef] text-[#0d5d39]":"border-[#d7e0e7] bg-white text-[#53657a]"}`}><span className="block text-sm font-extrabold">{option}</span><span className="mt-1 block text-xs">{option==="Free"?"Students can download directly":"Students see the Order Help action"}</span></button>)}</div></div>{draft.access==="Paid"?<div className="md:col-span-2"><TextField label="Price in PKR" type="number" value={draft.price} onChange={value=>update("price",value)} placeholder="850"/></div>:null}</div></section>
