...
export function MaterialListing({ category, items, initialQuery="" }: { category?: string; items: PublicMaterial[]; initialQuery?:string }) {
  const [query, setQuery] = useState(initialQuery); const [program, setProgram] = useState("All"); const [access, setAccess] = useState("All");
