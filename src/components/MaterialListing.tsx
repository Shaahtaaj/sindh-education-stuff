...
export function MaterialListing({ category }: { category?: string }) {
  const [query, setQuery] = useState(""); const [program, setProgram] = useState("All"); const [access, setAccess] = useState("All");
  const filtered = useMemo(() => materials.filter(x => (!category || x.category === category) && (program === "All" || x.program === program) && (access === "All" || (access === "Free" ? x.isFree : !x.isFree)) && `${x.title} ${x.courseCode} ${x.courseName} ${x.tags.join(" ")}`.toLowerCase().includes(query.toLowerCase())), [query, program, access, category]);
